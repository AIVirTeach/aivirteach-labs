...
: "${AIVIRTEACH_API_TOKEN:?Set AIVIRTEACH_API_TOKEN before starting the gateway}"
: "${AIVIRTEACH_DIAGNOSTIC_TOKEN:?Set AIVIRTEACH_DIAGNOSTIC_TOKEN before starting the gateway}"
...
cd "$PROJECT_DIR"
exec "$PYTHON_BIN" -m uvicorn gateway_service:app --host "$API_HOST" --port "$API_PORT"
